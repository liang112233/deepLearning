...
class decoder(nn.Module):
    def __init__(self, hidden_size, output_size, vocab_size, word_dim, helper=None, dropout_ratio=0.3):
        ...
        self.dropout = nn.Dropout(dropout_ratio)
        self.rnn = nn.LSTM(hidden_size+word_dim, hidden_size, batch_first=True)
    ...
    def forward(self, encoder_last_hidden_state, encoder_output, targets=None, mode='train', tr_steps=None):
        ...
        for i in range(seq_len-1): # only the MAX_SEQ_LEN-1 words will be the gru input
            ...
            gru_output, decoder_current_hidden_state = self.rnn(gru_input, decoder_current_hidden_state)
...
class MODELS(nn.Module):
    ...
    def forward(self, avi_feats, mode, target_sentences=None, tr_steps=None):
        encoder_outputs, (encoder_last_hidden_state,cell_last_state) = self.encoder(avi_feats)
        if mode == 'train':
